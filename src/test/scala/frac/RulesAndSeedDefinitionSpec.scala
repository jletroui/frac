...
class RulesAndSeedDefinitionSpec extends SpecificationWithJUnit
{
    val renderer = new StringRenderer

    "a rules and seed definition" should {
        "output seed when given no rules" in {
            val sut = new RulesAndSeedDefinition("ABC", Map.empty[Char, String])

            renderer.render(sut, 2) must beEqualTo("ABC")
        }
        "apply rules" in {
            val sut = new RulesAndSeedDefinition("AB", Map('A' -> "C", 'B' -> "D"))

            renderer.render(sut, 2) must beEqualTo("CD")
        }
        "recurse once when depth is one" in {
            val sut = new RulesAndSeedDefinition("AB", Map('A' -> "AA"))
